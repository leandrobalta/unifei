...
class LimiteConsultaDisc(tk.Toplevel):
    def __init__(self, controle):
        ...
        self.labelNro = tk.Label(self.frameNro,text="Codigo Disciplina: ")
        ...
        self.buttonSubmit = tk.Button(self.frameButton ,text="Enter")      
        self.buttonSubmit.pack(side="left")
        self.buttonSubmit.bind("<Button>", controle.searchHandler) 

        self.buttonFecha = tk.Button(self.frameButton ,text="Concluído")      
        self.buttonFecha.pack(side="left")
        self.buttonFecha.bind("<Button>", controle.fechaConsultHandler)
...
class CtrlDisciplina():       
    ...
    def consultaDisciplinas(self):
        self.consulta = LimiteConsultaDisc(self)
    ...
    def searchHandler(self, event):
        nroToSearch = self.consulta.inputNro.get()

        for disc in self.listaDisciplinas:
            if(nroToSearch == disc.getCodigo()):
                self.consulta.mostraJanela('Disciplina Encontrada!', f'Nome: {disc.getNome()}\nCodigo: {disc.getCodigo()}')
                return
        
        self.consulta.mostraJanela('Ops! Disciplina não encontrada', f'Nenhuma disciplina cadastrada possui o codigo [{nroToSearch}]')
