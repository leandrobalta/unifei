...
class CtrlEstudante():       
    ...
    def consultarEstudante(self):
        self.consultaEst = LimiteConsultaEstudante(self)
    ...
    def searchHandler(self, event):
        nroToSearch = self.consultaEst.inputNro.get()

        for est in self.listaEstudantes:
            if(nroToSearch == est.getNroMatric()):
                self.consultaEst.mostraJanela('Estudante Encontrado!', f'Nome: {est.getNome()}\nMatricula: {est.getNroMatric()}')
                return
        
        self.consultaEst.mostraJanela('Ops! Estudante não encontrado', f'Numero de Matricula [{nroToSearch}]\nNão esta cadastrado a nenhum estudante.')
