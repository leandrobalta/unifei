...
class CtrlTurma():       
    ...
    def consultaTurma(self):
        self.consulta = LimiteConsultaTurma(self)
    ...
    def searchHandler(self, event):
        ...
        for turma in self.listaTurmas:
            if(nroToSearch == turma.getDisciplina().getCodigo()):
        ...
        if(str != ''):
            self.consulta.mostraJanela('Turma Encontrada!', str)
            return
        self.consulta.mostraJanela('Ops! Turma não encontrada', f'Nenhuma turma cadastrada possui a disciplina [{nroToSearch}]')
