/*
Sistemas Operacionais - EP03

Nome: João Lucas Moraes de Oliveira     Matrícula: 2022004242
Nome: Leandro Balta Braga               Matrícula: 2022004260
*/

#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>
#include <unistd.h>
#include <semaphore.h>

#define C 7
#define N 20

pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;

// DECLARA SEMÁFORO PASSAGEIRO
sem_t passageiro;

// DECLARA SEMÁFORO CARRINHO
sem_t carrinho;


// DECLARA A QNTD DE PASSAGEIROS COMO GLOBAL
int Npass = 0;

void *Passageiro(void *arg)
{
    while (1)
    {
        sem_wait(&passageiro);
        
        pthread_mutex_lock(&mutex);
        Npass++;
        printf("Passageiro %d entrou no carrinho...\n", Npass);
        pthread_mutex_unlock(&mutex);

        sem_post(&carrinho);
    }
}

void *Carrinho(void *arg)
{
    while (1)
    {
        while (Npass < C)
        {
            printf("Carrinho esperando passageiros...\n");
            sem_wait(&carrinho);
        }

        pthread_mutex_lock(&mutex);
        printf("Carrinho partiu com %d passageiros...\n", Npass);
        sleep(2);
        Npass -= C;
        printf("Carrinho voltou e descarregou os passageiros...\n");
        pthread_mutex_unlock(&mutex);

        for (int i = 0; i < C; i++)
        {
            sem_post(&passageiro);
        }
    }
}

int main(void)
{
    // CRIANDO AS THREADS
    pthread_t passageiros[N];
    pthread_t thread_carrinho;

    sem_init(&passageiro, 0, C);
    sem_init(&carrinho, 0, 0);

    for (int i = 0; i < N; i++)
    {
        pthread_create(&passageiros[i], NULL, Passageiro, NULL);
    }

    pthread_create(&thread_carrinho, NULL, Carrinho, NULL);

    for (int i = 0; i < N; i++)
    {
        pthread_join(passageiros[i], NULL);
    }

    pthread_join(thread_carrinho, NULL);

    sem_destroy(&passageiro);
    sem_destroy(&carrinho);

    return 0;
}
